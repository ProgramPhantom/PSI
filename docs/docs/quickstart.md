...
Pulse Planner uses Figma-style diagram navigation.

- **Pan** the diagram with `MIDDLE MOUSE` drag, `SPACE + LEFT` or with two finger gestures on a trackpad.
- **Zoom** with `CTRL + MIDDLE MOUSE` or pinching on trackpad.
- **Select** with left click. Select an area with left click and drag.
...
![alt text](../static/video/draggingYourFirstPulse.gif)
...
![alt text](../static/video/addAChannel.gif)
...
![alt text](../static/video/deleteAnElement.gif)
...
![docs/static/video/modifyColumns.gif](../static/video/modifyColumns.gif)